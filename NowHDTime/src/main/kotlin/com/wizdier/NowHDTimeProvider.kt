...
class NowHDTimeProvider : MainAPI() {
    ...
    /* ── Home page sections (15 categories) ── */
    override val mainPage = mainPageOf(
        "$mainUrl/page/"                          to "Latest Updates",
        "$mainUrl/category/bangla-dubbed-movie/"  to "Bangla Dubbed",
        "$mainUrl/category/bangla-movie/"         to "Bangla Movies",
        "$mainUrl/category/bollywood/"            to "Bollywood",
        "$mainUrl/category/hollywood/"            to "Hollywood",
        "$mainUrl/category/south-indian-movie/"   to "South Indian",
        "$mainUrl/category/tamil-movie/"          to "Tamil Movies",
        "$mainUrl/category/korean-movie/"         to "Korean Movies",
        "$mainUrl/category/chinese-movie/"        to "Chinese Movies",
        ...
        "$mainUrl/category/turkish-drama/"        to "Turkish Drama",
    ...
    private fun parsePageItems(document: Document): List<SearchResponse> {
        ...
        for (selector in selectors) {
            ...
            if (elements.isNotEmpty()) {
                elements.forEach { el -> parseArticle(el)?.let { results.add(it) } }
                if (results.isNotEmpty()) break
            }
        }

        // Fallback: thumbnail links inside content area
        if (results.isEmpty()) {
    ...
    private fun parseArticle(element: Element): SearchResponse? {
        ...
        if (href.isBlank() || !href.startsWith(mainUrl) || href == mainUrl ||
            href.contains("/category/") || href.contains("/tag/") ||
            href.contains("/page/")  || href.contains("/author/")) return null
    ...
    private fun detectType(url: String, title: String): TvType {
        ...
        return when {
            ...
            c.contains("drama") || c.contains("kdrama") || c.contains("cdrama") ||
                c.contains("korean-drama") || c.contains("turkish") -> TvType.AsianDrama
            else -> TvType.Movie
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val year = NowHDTimeUtils.extractYear(rawTitle)
            ?: extractMetaField(document, listOf("Release", "Year", "Released"))
                ?.let { NowHDTimeUtils.extractYear(it) }
        ...
        val duration = extractMetaField(document, listOf("Runtime", "Duration", "Time"))
            ?.let { NowHDTimeUtils.parseDuration(it) }
        val rating = extractMetaField(document, listOf("IMDB", "Rating", "IMDb Rating"))
            ?.let { NowHDTimeUtils.extractRating(it) }
    ...
    private fun extractMetaField(document: Document, fieldNames: List<String>): String? {
        val content = document.selectFirst(".entry-content, .post-content, article, .movie-info")
            ?: document
            ...
            for (selector in selectors) {
                for (element in content.select(selector)) {
                    ...
                    if (next != null) {
                        ...
                        if (nextText.isNotBlank() && nextText.length < 500) return nextText
                    }
                }
            }
        }
    ...
    private fun extractTrailerUrl(document: Document): String? {
        document.selectFirst("iframe[src*=youtube], iframe[src*=vimeo]")?.let { iframe ->
            return iframe.attr("src").let {
                if (it.startsWith("//")) "https:$it" else it
            }
        }
        Regex("(?:youtube\.com/embed/|youtu\.be/)([A-Za-z0-9_-]{11})")
            .find(document.html())?.groupValues?.get(1)?.let { id ->
    ...
    private suspend fun extractEpisodes(document: Document, pageUrl: String): List<Episode> {
        ...
        val content = document.selectFirst(".entry-content, .post-content, article")
            ?: return listOf(
                newEpisode(pageUrl) {
        ...
        val epLinks = content.select("a[href*='episode'], a[href*='ep-'], a[href*='ep.']") +
        ...
        // Method 2: season / episode headings with links below
        if (episodes.isEmpty()) {
    ...
    override suspend fun loadLinks(
        ...
        // 5. WP Ajax / player API
        extractPlayerAPILinks(document, data, callback, subtitleCallback)?.let { found = true }

        // 6. Fallback — any external link in content
        if (!found) {
            val contentLinks = document.select(".entry-content a[href], .post-content a[href], article a[href]")
            for (link in contentLinks) {
                val href = link.attr("href")
                if (href.startsWith("http") && !href.contains("nowhdtime.com.bd") &&
                    !href.contains("t.me") && !href.contains("facebook") &&
                    !href.contains("twitter")  && !href.contains("instagram") &&
    ...
    private fun extractJSLinks(
        ...
        Regex("""(?i)(?:tracks|subtitles|captions)\s*[:=]\s*\[([^\]]+)]""").find(html)?.let { trackMatch ->
            ...
            Regex("""(?i)(?:file|src)\s*:\s*["']([^"']+\.(?:srt|vtt|ass)[^"']*)["']""")
                .findAll(tracks).forEach { sub ->
                    val lang = Regex("""(?i)label\s*:\s*["']([^"']+)["']""")
                        .find(tracks)?.groupValues?.get(1) ?: "Unknown"
