...
class CircleFtpProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val posts = parsePageData(JSONObject(rawPage)).posts
    ...
    private suspend fun loadAnimeMovie(
        ...
        return@coroutineScope newMovieLoadResponse(
            ...
            type = mediaType,
            dataUrl = streamPayload.toJsonString(),
        ) {
            this.posterUrl = animeMeta?.posterUrl ?: fallbackPoster
            this.backgroundPosterUrl = animeMeta?.backgroundPosterUrl ?: fallbackPoster
            this.logoUrl = animeMeta?.logoUrl
    ...
    private suspend fun fetchPostDetails(ref: PayloadPostRef): FetchedPost {
        ...
        val data = parseData(JSONObject(apiResponse.text))
        return if (data.type == "singleVideo") {
            val movieData = parseMovies(JSONObject(apiResponse.text))
            FetchedPost(
                ref = ref,
                data = data,
            ...
            FetchedPost(
                ...
                data = data,
                movieLink = null,
                tvSeries = parseTvSeries(JSONObject(apiResponse.text)),
                usedFallbackApi = apiResponse.usedFallback,
            )
        }
    }
    ...
    private suspend fun fetchFribbAnimeEntry(anilistId: Int): FribbAnimeEntry? {
        ...
        val parsedEntries = parseFribbList(JSONArray(responseText))
    ...
    private fun encodePayload(payload: CirclePayload): String {
        val rawJson = payload.toJsonString().toByteArray()
    ...
    // ==============================
    // JSON Parser Helpers (org.json)
    // ==============================
    private fun parsePageData(json: JSONObject): PageData {
        val postsArray = json.optJSONArray("posts") ?: return PageData()
        val posts = (0 until postsArray.length()).map { i ->
            val post = postsArray.getJSONObject(i)
            Post(
                id = post.optInt("id"),
                type = post.optString("type"),
                imageSm = post.optString("imageSm").takeIf { it.isNotEmpty() },
                title = post.optString("title"),
                name = post.optString("name").takeIf { it.isNotEmpty() },
                year = post.optString("year").takeIf { it.isNotEmpty() },
                quality = post.optString("quality").takeIf { it.isNotEmpty() }
            )
        }
        return PageData(posts = posts)
    }

    private fun parseData(json: JSONObject): Data {
        return Data(
            type = json.optString("type"),
            imageSm = json.optString("imageSm").takeIf { it.isNotEmpty() },
            title = json.optString("title"),
            image = json.optString("image").takeIf { it.isNotEmpty() },
            metaData = json.optString("metaData").takeIf { it.isNotEmpty() },
            name = json.optString("name").takeIf { it.isNotEmpty() },
            quality = json.optString("quality").takeIf { it.isNotEmpty() },
            year = json.optString("year").takeIf { it.isNotEmpty() },
            watchTime = json.optString("watchTime").takeIf { it.isNotEmpty() }
        )
    }

    private fun parseMovies(json: JSONObject): Movies {
        return Movies(content = json.optString("content").takeIf { it.isNotEmpty() })
    }

    private fun parseTvSeries(json: JSONObject): TvSeries {
        val contentArray = json.optJSONArray("content") ?: return TvSeries()
        val content = (0 until contentArray.length()).map { i ->
            ...
            val episodes = (0 until episodesArray.length()).map { j ->
                val epObj = episodesArray.getJSONObject(j)
                EpisodeData(
                    link = epObj.optString("link"),
                    title = epObj.optString("title").takeIf { it.isNotEmpty() }
                )
            }
            SeasonContent(
                episodes = episodes,
                seasonName = seasonObj.optString("seasonName").takeIf { it.isNotEmpty() }
            )
        }
        return TvSeries(content = content)
    }

    private fun parseTmdbConfiguration(json: JSONObject): TmdbConfiguration {
        val imagesObj = json.optJSONObject("images")
        return TmdbConfiguration(
            images = if (imagesObj != null) TmdbImageConfig(
                secureBaseUrl = imagesObj.optString("secure_base_url").takeIf { it.isNotEmpty() },
                posterSizes = imagesObj.optJSONArray("poster_sizes")?.let { arr ->
                    (0 until arr.length()).map { arr.getString(it) }
                },
                backdropSizes = imagesObj.optJSONArray("backdrop_sizes")?.let { arr ->
                    (0 until arr.length()).map { arr.getString(it) }
                },
                logoSizes = imagesObj.optJSONArray("logo_sizes")?.let { arr ->
                    (0 until arr.length()).map { arr.getString(it) }
                },
                profileSizes = imagesObj.optJSONArray("profile_sizes")?.let { arr ->
                    (0 until arr.length()).map { arr.getString(it) }
                }
            ) else null
        )
    ...
    private fun parseTmdbSearchPage(json: JSONObject): TmdbSearchPage {
        ...
        return TmdbSearchPage(results = results)
    }
    ...
    private fun parseTmdbTvDetails(json: JSONObject): TmdbTvDetails {
        return TmdbTvDetails(
            ...
            episodeRunTime = json.optJSONArray("episode_run_time")?.let { arr ->
                (0 until arr.length()).map { arr.getInt(it) }
            },
            voteAverage = json.optDouble("vote_average").takeIf { !it.isNaN() },
            genres = json.optJSONArray("genres")?.let { arr ->
                (0 until arr.length()).map { TmdbGenre(name = arr.getJSONObject(it).optString("name").takeIf { n -> n.isNotEmpty() }) }
            },
    ...
    private fun parseAniListSearchResponse(json: JSONObject): AniListSearchResponse {
        ...
        val media = (0 until mediaArray.length()).map { i -> parseAniListMedia(mediaArray.getJSONObject(i)) }
        return AniListSearchResponse(data = AniListSearchData(page = AniListPage(media = media)))
    }

    private fun parseAniListByIdResponse(json: JSONObject): AniListByIdResponse {
        val dataObj = json.optJSONObject("data") ?: return AniListByIdResponse()
        val media = dataObj.optJSONObject("Media")?.let { parseAniListMedia(it) }
        return AniListByIdResponse(data = AniListByIdData(media = media))
    }

    private fun parseAniListMedia(json: JSONObject): AniListMedia {
        return AniListMedia(
            id = json.optInt("id").takeIf { it > 0 },
            idMal = json.optInt("idMal").takeIf { it > 0 },
            title = json.optJSONObject("title")?.let { titleObj ->
                AniListTitle(
                    romaji = titleObj.optString("romaji").takeIf { it.isNotEmpty() },
                    english = titleObj.optString("english").takeIf { it.isNotEmpty() },
                    native = titleObj.optString("native").takeIf { it.isNotEmpty() },
                    userPreferred = titleObj.optString("userPreferred").takeIf { it.isNotEmpty() }
                )
            },
            ...
            status = json.optString("status").takeIf { it.isNotEmpty() },
            seasonYear = json.optInt("seasonYear").takeIf { it > 0 },
            startDate = json.optJSONObject("startDate")?.let { dateObj ->
                AniListDate(
                    year = dateObj.optInt("year").takeIf { it > 0 },
                    month = dateObj.optInt("month").takeIf { it > 0 },
                    day = dateObj.optInt("day").takeIf { it > 0 }
                )
            },
            episodes = json.optInt("episodes").takeIf { it > 0 },
            duration = json.optInt("duration").takeIf { it > 0 },
            averageScore = json.optInt("averageScore").takeIf { it > 0 },
            description = json.optString("description").takeIf { it.isNotEmpty() },
            bannerImage = json.optString("bannerImage").takeIf { it.isNotEmpty() },
            genres = json.optJSONArray("genres")?.let { arr ->
                (0 until arr.length()).map { arr.getString(it) }
            },
            trailer = json.optJSONObject("trailer")?.let { trailerObj ->
                AniListTrailer(
                    id = trailerObj.optString("id").takeIf { it.isNotEmpty() },
                    site = trailerObj.optString("site").takeIf { it.isNotEmpty() },
                    thumbnail = trailerObj.optString("thumbnail").takeIf { it.isNotEmpty() }
                )
            },
            coverImage = json.optJSONObject("coverImage")?.let { coverObj ->
                AniListCoverImage(
                    extraLarge = coverObj.optString("extraLarge").takeIf { it.isNotEmpty() },
                    large = coverObj.optString("large").takeIf { it.isNotEmpty() }
                )
            },
            relations = json.optJSONObject("relations")?.let { relObj ->
                val edgesArray = relObj.optJSONArray("edges") ?: return@let null
                AniListRelations(
                    edges = (0 until edgesArray.length()).map { i ->
                        val edgeObj = edgesArray.getJSONObject(i)
                        AniListRelationEdge(
                            relationType = edgeObj.optString("relationType"),
                            node = edgeObj.optJSONObject("node")?.let { nodeObj -> parseAniListMedia(nodeObj) } ?: AniListMedia()
                        )
                    }
                )
            }
    ...
    private fun parseAniZipResponse(json: JSONObject): AniZipResponse {
        return AniZipResponse(
            ...
            episodes = json.optJSONObject("episodes")?.let { epObj ->
                ...
                epObj.keys().forEach { key ->
                    ...
                    map[key] = AniZipEpisode(
                        title = ep.optJSONObject("title")?.let { titleObj ->
                            AniZipEpisodeTitle(
                                en = titleObj.optString("en").takeIf { it.isNotEmpty() },
                                ja = titleObj.optString("ja").takeIf { it.isNotEmpty() },
                                xJat = titleObj.optString("x-jat").takeIf { it.isNotEmpty() }
                            )
                        },
                        ...
                        runtime = ep.optInt("runtime").takeIf { it > 0 },
                        length = ep.optInt("length").takeIf { it > 0 },
            ...
            mappings = json.optJSONObject("mappings")?.let { mapObj ->
                AniZipMappings(
                    anilistId = mapObj.optInt("anilist_id").takeIf { it > 0 },
                    malId = mapObj.optInt("mal_id").takeIf { it > 0 },
                    kitsuId = mapObj.optInt("kitsu_id").takeIf { it > 0 },
                    simklId = mapObj.optInt("simkl_id").takeIf { it > 0 },
                    imdbId = mapObj.optString("imdb_id").takeIf { it.isNotEmpty() },
                    tmdbId = mapObj.optJSONObject("themoviedb_id")?.let { tmdbObj ->
                        AniZipTmdbId(
                            movie = tmdbObj.optInt("movie").takeIf { it > 0 },
                            tv = tmdbObj.optInt("tv").takeIf { it > 0 }
                        )
                    }
                )
            }
    ...
    private fun parseFribbList(json: JSONArray): List<FribbAnimeEntry> {
        return (0 until json.length()).map { i ->
            val obj = json.getJSONObject(i)
            FribbAnimeEntry(
                anilistId = obj.optInt("anilist_id").takeIf { it > 0 },
                malId = obj.optInt("mal_id").takeIf { it > 0 },
                kitsuId = obj.optInt("kitsu_id").takeIf { it > 0 },
                simklId = obj.optInt("simkl_id").takeIf { it > 0 },
                imdbId = obj.optString("imdb_id").takeIf { it.isNotEmpty() },
                tmdbId = obj.optJSONObject("themoviedb_id")?.let { tmdbObj ->
                    AniZipTmdbId(
                        movie = tmdbObj.optInt("movie").takeIf { it > 0 },
                        tv = tmdbObj.optInt("tv").takeIf { it > 0 }
                    )
    ...
    private fun StreamPayload.toJsonString(): String {
        ...
        json.put("variants", variantsArray)
        return json.toString()
    }

    private fun CirclePayload.toJsonString(): String {
        val json = JSONObject()
        json.put("mediaType", mediaType)
        json.put("displayTitle", displayTitle)
        json.put("franchiseTitle", franchiseTitle)
        year?.let { json.put("year", it) }
        activeSeason?.let { json.put("activeSeason", it) }
        val postsArray = JSONArray()
        posts.forEach { post ->
            postsArray.put(JSONObject().apply {
                put("id", post.id)
                put("rawTitle", post.rawTitle)
                post.imageSm?.let { put("imageSm", it) }
                post.declaredSeason?.let { put("declaredSeason", it) }
                post.audioTag?.let { put("audioTag", it) }
                post.postType?.let { put("postType", it) }
            })
        }
