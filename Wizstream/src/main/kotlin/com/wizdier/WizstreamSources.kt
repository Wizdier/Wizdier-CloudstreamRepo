...
object WizstreamSources {
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  Resolver 6: Aether  (https://aether.bar)
    //
    //  Recon (2026-07-21): Aether is a P-Stream/movie-web fork whose streams
    //  come from two tiny normalize-APIs (TMDB-keyed, perfect for Cloudstream
    //  since we always have tmdbId):
    //
    //    Link:  GET https://link.aether.cx/movie/{tmdbId}
    //           GET https://link.aether.cx/tv/{tmdbId}/{season}/{episode}
    //           → {"id":…,"title":"… YYYY","stream":"{m3u8-proxy-wrapped url}"}
    //           The wrapper is <cdn>/m3u8-proxy?url={urlenc master.m3u8}
    //           &headers={urlenc {"Referer":…,"Origin":…}} — we UNWRAP it and
    //           emit the direct HLS with its own referer/origin (avoids the
    //           proxy entirely; direct paths verified playable).
    //           Catalogue is H.264/avc1 SDR up to 1080p; variants are emitted
    //           per-quality WITH codec tags (our HTTP-4003 guard from v18).
    //
    //    Lul:   same paths on lul.aether.cx (their fallback scraper wrapping
    //           play.xpass.top). Frequently rate-limited — kept as best-effort
    //           second source: it simply resolves nothing while limited.
    //
    //    Subs:  sub.vdrk.site/v1/movie/{tmdb} and /v1/tv/{tmdb}/{s}/{e} —
    //           the same free subtitle API the site itself queries; returns
    //           [{label, file.vtt}]. We emit English entries first.
    //
    //  All links are probe-validated by emitMediaCandidates before reaching
    //  the picker, so dead/CDN-blocked URLs can never cause player HTTP
    //  2004 (bad status) or 3003 (HTML-as-playlist) failures.
    // ════════════════════════════════════════════════════════════════════════

    internal object AetherResolver : SourceResolver {
        private const val LINK_API = "https://link.aether.cx"
        private const val LUL_API = "https://lul.aether.cx"
        private const val VDRK_API = "https://sub.vdrk.site"
        private const val SITE = "https://aether.bar"
        private const val LABEL = "Aether"
        private val HEADERS = mapOf(
            "User-Agent" to UA,
            "Origin" to SITE,
            "Referer" to "$SITE/",
        ...
        override suspend fun resolve(
            ...
            // Aether keys everything by TMDB id — we require it.
            if (tmdbId == null) return false
            if (!isMovie && (season == null || episode == null)) return false
            val srcLabel = "$labelPrefix • $LABEL"

            // Subtitles (independent of stream resolution).
            emitVdrkSubs(app, isMovie, tmdbId, season, episode, subtitleCallback)

            val cands = mutableListOf<WizstreamAnimeSources.MediaCandidate>()
            scrapeAether(app, LINK_API, "Link", isMovie, tmdbId, season, episode, srcLabel, cands)
            scrapeAether(app, LUL_API, "Lul", isMovie, tmdbId, season, episode, srcLabel, cands)
            if (cands.isEmpty()) return false
            return WizstreamAnimeSources.emitMediaCandidates(cands, subtitleCallback, callback)
        }

        private suspend fun scrapeAether(
            app: Requests,
            base: String,
            serverTag: String,
            isMovie: Boolean,
            ...
            episode: Int?,
            srcLabel: String,
            out: MutableList<WizstreamAnimeSources.MediaCandidate>,
        ) {
            val path = if (isMovie) "/movie/$tmdbId" else "/tv/$tmdbId/$season/$episode"
            val body = runCatching {
                app.get(base + path, headers = HEADERS, timeout = 20_000).text
            }.getOrNull() ?: return
            val json = runCatching { JSONObject(body) }.getOrNull() ?: return
            val wrapped = json.optStringOrNullCp("stream") ?: return
            val (directUrl, streamHeaders) = unwrapAetherProxy(wrapped)
            if (directUrl.isBlank()) return
            val referer = streamHeaders["Referer"] ?: SITE

            // Split the master into per-quality codec-tagged candidates.
            val variants = fetchHlsVariants(app, directUrl, streamHeaders)
            if (variants.isEmpty()) {
                out += WizstreamAnimeSources.MediaCandidate(
                    url = directUrl, sourceLabel = srcLabel,
                    name = "$srcLabel · $serverTag",
                    referer = referer, headers = streamHeaders, forceHls = true,
                )
            } else {
                for (v in variants) {
                    out += WizstreamAnimeSources.MediaCandidate(
                        url = v.second, sourceLabel = srcLabel,
                        name = "$srcLabel · $serverTag · ${v.first}",
                        referer = referer, headers = streamHeaders, forceHls = true,
                        quality = qualityFromName(v.first),
                    )
                }
            }
        }

        /** m3u8-proxy wrapper → (direct master url, required headers). */
        private fun unwrapAetherProxy(stream: String): Pair<String, Map<String, String>> {
            return runCatching {
                if ("/m3u8-proxy" in stream && "?url=" in stream) {
                    val query = stream.substringAfter("?")
                    val params = query.split("&").associate {
                        it.substringBefore("=") to java.net.URLDecoder.decode(
                            it.substringAfter("=", ""), Charsets.UTF_8.name()
                        )
                    }
                    val target = params["url"]
                    if (!target.isNullOrBlank()) {
                        val hdrs = params["headers"]?.let { h ->
                            runCatching {
                                val o = JSONObject(h)
                        ...
                        return target to hdrs
                    }
                }
                stream to mapOf("User-Agent" to UA)
            }.getOrDefault(stream to mapOf("User-Agent" to UA))
        }

        /** Parse EXT-X-STREAM-INF blocks → (display label, absolute url). */
        private suspend fun fetchHlsVariants(
            app: Requests,
            masterUrl: String,
            headers: Map<String, String>,
            ...
            val text = runCatching {
                app.get(
                    masterUrl,
                    headers = headers.ifEmpty { mapOf("User-Agent" to UA) },
                    timeout = 15_000,
                ).text
            }.getOrNull() ?: return emptyList()
            if (!text.contains("#EXTM3U")) return emptyList()
            val out = mutableListOf<Pair<String, String>>()
            val lines = text.lines()
            for (i in lines.indices) {
                val line = lines[i].trim()
                if (!line.startsWith("#EXT-X-STREAM-INF")) continue
                val child = lines.getOrNull(i + 1)?.trim() ?: continue
                if (child.startsWith("#")) continue
                val res = Regex("""RESOLUTION=\d+x(\d+)""").find(line)
                    ?.groupValues?.getOrNull(1)?.toIntOrNull()
                val codecs = Regex("""CODECS="([^"]+)"""").find(line)
                    ?.groupValues?.getOrNull(1) ?: ""
                if (res == null) continue
                val tag = aetherCodecTag(codecs)
                val abs = if (child.startsWith("http")) child
                    else masterUrl.substringBefore("://") + "://" +
                        masterUrl.substringAfter("://").substringBefore("/") +
                        if (child.startsWith("/")) child else "/" + child
                out += "${res}p$tag" to abs
            }
            // Highest quality first; cap at 6 variants.
            return out.sortedByDescending {
                it.first.substringBefore("p").toIntOrNull() ?: 0
            }.take(6)
        }

        /** Mirror of the v18 Cineby codec tags — the 4003 guard for old TVs. */
        private fun aetherCodecTag(codecs: String?): String {
            val c = (codecs ?: return " · H.264").lowercase()
            return when {
                c.startsWith("hev1") || c.startsWith("hvc1") ||
                    c.contains(",hev1") || c.contains(",hvc1") ||
                    c.startsWith("dvh1") || c.startsWith("dvhe") -> " · HEVC ⚠"
                c.startsWith("av01") || c.contains(",av01") -> " · AV1 ⚠"
                c.startsWith("vp09") || c.contains("vp09") || c.startsWith("vp9") -> " · VP9 ⚠"
                else -> " · H.264"
            }
        }

        /** sub.vdrk.site — the subtitle API Aether's player itself queries. */
        private suspend fun emitVdrkSubs(
            app: Requests,
            isMovie: Boolean,
            tmdbId: Int,
            season: Int?,
            ...
            val path = if (isMovie) "/v1/movie/$tmdbId"
                else if (season != null && episode != null) "/v1/tv/$tmdbId/$season/$episode"
                else return
            val text = runCatching {
                app.get(VDRK_API + path, headers = HEADERS, timeout = 12_000).text
            }.getOrNull() ?: return
            val arr = runCatching { JSONArray(text) }.getOrNull() ?: return
            val subs = (0 until arr.length()).mapNotNull { i ->
                val o = arr.optJSONObject(i) ?: return@mapNotNull null
                val label = o.optStringOrNullCp("label") ?: return@mapNotNull null
                val file = o.optStringOrNullCp("file") ?: return@mapNotNull null
                label to file
            }
            if (subs.isEmpty()) return
            val (en, rest) = subs.partition { it.first.contains("english", ignoreCase = true) }
            (en.take(6) + rest.take(10)).forEach { (label, file) ->
                // vdrk file paths contain literal spaces ("English Hi2.vtt") —
                // raw spaces are illegal in a URI request line, so encode them.
                val safeFile = file.trim().replace(" ", "%20")
                subtitleCallback(SubtitleFile("[Aether] $label", safeFile))
            }
        }
    }
}
